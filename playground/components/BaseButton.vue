...
const computedButtonIconSize = computed(() => props.small ? '16' : '20')
...
const computedButtonClasses = computed(() => {
	return {
		...
		'px-3 h-10': !props.small && !iconOnly.value,
		'px-2.5 h-8': props.small && !iconOnly.value,

		'flex items-center justify-center rounded': iconOnly.value,
		'h-10 w-10': !props.small && iconOnly.value,
		'h-8 w-8': props.small && iconOnly.value,
	}
})
</script>
	...
	>
		<span class="relative z-10 flex items-center">
			...
			<span
				v-if="!iconOnly"
				:class="{
					'mx-1.5': props.icon && props.iconRight,
					'mr-1.5': !props.icon && props.iconRight,
					'ml-1.5': props.icon && !props.iconRight,
				}"
			>
				<slot />
			</span>
